...
\subsection{\acl{CE}}
\acl{CE} is a measure that determines the similarity between two probability distributions. In the context of neural networks, it is commonly used as a loss function for classification tasks, where the predicted output of the network represents a probability distribution over different classes.
...
\begin{gather}
    ...
    \pdv{S_{ij}}{Z_{lt}} = S_{ij} \pdv{\log(S_{ij})}{Z_{lt}} = S_{ij} (\delta_{ij,lt} - S_{lt})
\end{gather}
...
\begin{split}
    ...
    &\overset{(a)}{=} -\frac{1}{m} \sum_{i=1}^{m} \sum_{j=1}^{C} Y_{ij} \left(\delta_{ij,lt} - S_{lt} \right)\\
    &= -\frac{1}{m} \left( Y_{lt} - S_{lt} \sum_{i=1}^{m} \sum_{j=1}^{C} Y_{ij} \right)\\
    &\overset{(b)}{=} -\frac{1}{m} \left( Y_{lt} - S_{lt} \sum_{i=1}^{m} 1 \right)\\
    &= -\frac{1}{m} \left( Y_{lt} - m S_{lt} \right)\\
    &= S_{lt} - \frac{Y_{lt}}{m}
\end{split}
\end{equation}
